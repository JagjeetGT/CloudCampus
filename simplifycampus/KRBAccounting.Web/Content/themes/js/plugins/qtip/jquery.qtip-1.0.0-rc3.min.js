/*
 * jquery.qtooltip. The jQuery tooltip plugin
 *
 ...
 * Debug: jquery.qtooltip.debug.js
 */
(function(f){
    f.fn.qtooltip=function(B,u){
        var y,t,A,s,x,w,v,z;
        if(typeof B=="string"){
            if(typeof f(this).data("qtooltip")!=="object"){
                f.fn.qtooltip.log.error.call(self,1,f.fn.qtooltip.constants.NO_TOOLTIP_PRESENT,false)
                }
                if(B=="api"){
                return f(this).data("qtooltip").interfaces[f(this).data("qtooltip").current]
                }else{
                if(B=="interfaces"){
                    return f(this).data("qtooltip").interfaces
                    }
                }
        }else{
    if(!B){
...
B.style=c(B.style);
s=f.extend(true,{},f.fn.qtooltip.defaults,B);
s.style=a.call({
    options:s
},s.style);
...
return f(this).each(function(){
    ...
    if(v.position.container===false){
        ...
        t=f.fn.qtooltip.interfaces.length;
    for(y=0;y<t;y++){
        if(typeof f.fn.qtooltip.interfaces[y]=="undefined"){
            t=y;
            break
        }
    }
    x=new d(f(this),v,t);
f.fn.qtooltip.interfaces[t]=x;
if(typeof f(this).data("qtooltip")=="object"){
    if(typeof f(this).attr("qtooltip")==="undefined"){
        f(this).data("qtooltip").current=f(this).data("qtooltip").interfaces.length
        }
        f(this).data("qtooltip").interfaces.push(x)
    }else{
    f(this).data("qtooltip",{
        current:0,
        interfaces:[x]
        })
    }
    if(v.content.prerender===false&&v.show.when.event!==false&&v.show.ready!==true){
    v.show.when.target.bind(v.show.when.event+".qtooltip-"+t+"-create",{
        qtooltip:t
    },function(C){
        z=f.fn.qtooltip.interfaces[C.data.qtooltip];
        z.options.show.when.target.unbind(z.options.show.when.event+".qtooltip-"+C.data.qtooltip+"-create");
        z.cache.mouse={
            x:C.pageX,
            y:C.pageY
            };
...
function d(u,t,v){
    ...
    f.extend(s,s.options.api,{
        show:function(y){
            ...
            if(!s.status.rendered){
                return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.TOOLTIP_NOT_RENDERED,"show")
                }
        ...
        if(s.options.position.type!=="static"){
            ...
            if(s.options.show.solo===true){
                z=f("div.qtooltip").not(s.elements.tooltip)
                }
            }
        if(z){
        z.each(function(){
            if(f(this).qtooltip("api").status.rendered===true){
                f(this).qtooltip("api").hide()
                }
            })
    }
    ...
    return f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.EVENT_SHOWN,"show")
},
hide:function(y){
    var x;
    if(!s.status.rendered){
        return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.TOOLTIP_NOT_RENDERED,"hide")
        }else{
        if(s.elements.tooltip.css("display")==="none"){
            return s
            }
...
if(typeof s.options.hide.effect.type=="function"){
    ...
    return f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.EVENT_HIDDEN,"hide")
},
updatePosition:function(w,x){
    var C,G,L,J,H,E,y,I,B,D,K,A,F,z;
    if(!s.status.rendered){
        return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.TOOLTIP_NOT_RENDERED,"updatePosition")
        }else{
        if(s.options.position.type=="static"){
            return f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.CANNOT_POSITION_STATIC,"updatePosition")
            }
        }
    G={
    position:{
...
if(s.options.position.target!=="mouse"){
    ...
    break;
default:
    return f.fn.qtooltip.log.error.call(s,4,f.fn.qtooltip.constants.INVALID_AREA_SHAPE,"updatePosition");
    break
    }
...
G.dimensions.height-=2
    ...
    if(typeof s.options.position.target.attr("qtooltip")!=="undefined"){
        G.position=s.options.position.target.qtooltip("api").cache.position
        }else{
...
if(y.left!=F.left||y.top!=F.top){
    ...
    if(typeof w!=="undefined"&&w.type&&w.type!=="mousemove"){
        f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.EVENT_POSITION_UPDATED,"updatePosition")
        }
    }
return s
},
updateWidth:function(w){
    var x;
    if(!s.status.rendered){
        return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.TOOLTIP_NOT_RENDERED,"updateWidth")
        }else{
        if(w&&typeof w!=="number"){
            return f.fn.qtooltip.log.error.call(s,2,"newWidth must be of type number","updateWidth")
            }
        }
...
if(s.options.style.border.radius){
    s.elements.tooltip.find(".qtooltip-betweenCorners").each(function(y){
...
return f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.EVENT_WIDTH_UPDATED,"updateWidth")
},
updateStyle:function(w){
    var z,A,x,y,B;
    if(!s.status.rendered){
        return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.TOOLTIP_NOT_RENDERED,"updateStyle")
        }else{
        if(typeof w!=="string"||!f.fn.qtooltip.styles[w]){
            return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.STYLE_NOT_DEFINED,"updateStyle")
            }
        }
    s.options.style=a.call(s,f.fn.qtooltip.styles[w],s.options.user.style);
...
if(s.options.style.tip.corner!==false){
    if(f("<canvas>").get(0).getContext){
        z=s.elements.tooltip.find(".qtooltip-tip canvas:first");
        ...
        if(f.browser.msie){
            z=s.elements.tooltip.find('.qtooltip-tip [nodeName="shape"]');
            z.attr("fillcolor",s.options.style.tip.color||s.options.style.border.color)
            }
        }
}
if(s.options.style.border.radius>0){
    s.elements.tooltip.find(".qtooltip-betweenCorners").css({
        backgroundColor:s.options.style.border.color
        });
    if(f("<canvas>").get(0).getContext){
        A=g(s.options.style.border.radius);
        s.elements.tooltip.find(".qtooltip-wrapper canvas").each(function(){
        ...
        if(f.browser.msie){
            s.elements.tooltip.find('.qtooltip-wrapper [nodeName="arc"]').each(function(){
                f(this).attr("fillcolor",s.options.style.border.color)
                })
            }
        }
}
return f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.EVENT_STYLE_UPDATED,"updateStyle")
},
updateContent:function(A,y){
    var z,x,w;
    if(!s.status.rendered){
        return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.TOOLTIP_NOT_RENDERED,"updateContent")
        }else{
        if(!A){
            return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.NO_CONTENT_PROVIDED,"updateContent")
            }
        }
...
return f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.EVENT_CONTENT_UPDATED,"loadContent")
},
loadContent:function(w,z,A){
    var y;
    if(!s.status.rendered){
        return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.TOOLTIP_NOT_RENDERED,"loadContent")
        }
    ...
    if(y===false){
        ...
        f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.EVENT_CONTENT_LOADED,"loadContent");
        s.updateContent(B)
        }
        return s
    },
updateTitle:function(w){
    if(!s.status.rendered){
        return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.TOOLTIP_NOT_RENDERED,"updateTitle")
        }else{
        if(!w){
            return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.NO_CONTENT_PROVIDED,"updateTitle")
            }
        }
...
return f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.EVENT_TITLE_UPDATED,"updateTitle")
},
focus:function(A){
    var y,x,w,z;
    if(!s.status.rendered){
        return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.TOOLTIP_NOT_RENDERED,"focus")
        }else{
        if(s.options.position.type=="static"){
            return f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.CANNOT_FOCUS_STATIC,"focus")
            }
        }
    y=parseInt(s.elements.tooltip.css("z-index"));
x=6000+f("div.qtooltip[qtooltip]").length-1;
if(!s.status.focused&&y!==x){
    z=s.beforeFocus.call(s,A);
    if(z===false){
        return s
        }
        f("div.qtooltip[qtooltip]").not(s.elements.tooltip).each(function(){
        if(f(this).qtooltip("api").status.rendered===true){
            ...
            if(typeof w=="number"&&w>-1){
                ...
                f(this).qtooltip("api").status.focused=false
            }
        });
s.elements.tooltip.css({
    zIndex:x
...
s.onFocus.call(s,A);
f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.EVENT_FOCUSED,"focus")
}
return s
},
disable:function(w){
    if(!s.status.rendered){
        return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.TOOLTIP_NOT_RENDERED,"disable")
        }
        if(w){
        if(!s.status.disabled){
            s.status.disabled=true;
            f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.EVENT_DISABLED,"disable")
            }else{
            f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.TOOLTIP_ALREADY_DISABLED,"disable")
            }
        }else{
    if(s.status.disabled){
        s.status.disabled=false;
        f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.EVENT_ENABLED,"disable")
        }else{
        f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.TOOLTIP_ALREADY_ENABLED,"disable")
        }
    }
return s
},
destroy:function(){
    ...
    if(x===false){
        ...
        if(s.status.rendered){
        s.options.show.when.target.unbind("mousemove.qtooltip",s.updatePosition);
        s.options.show.when.target.unbind("mouseout.qtooltip",s.hide);
        s.options.show.when.target.unbind(s.options.show.when.event+".qtooltip");
        s.options.hide.when.target.unbind(s.options.hide.when.event+".qtooltip");
        s.elements.tooltip.unbind(s.options.hide.when.event+".qtooltip");
        s.elements.tooltip.unbind("mouseover.qtooltip",s.focus);
        s.elements.tooltip.remove()
        }else{
        s.options.show.when.target.unbind(s.options.show.when.event+".qtooltip-create")
        }
        if(typeof s.elements.target.data("qtooltip")=="object"){
        y=s.elements.target.data("qtooltip").interfaces;
...
delete f.fn.qtooltip.interfaces[s.id];
if(typeof y=="object"&&y.length>0){
    s.elements.target.data("qtooltip").current=y.length-1
    }else{
    s.elements.target.removeData("qtooltip")
    }
    s.onDestroy.call(s);
f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.EVENT_DESTROYED,"destroy");
return s.elements.target
},
getPosition:function(){
    var w,x;
    if(!s.status.rendered){
        return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.TOOLTIP_NOT_RENDERED,"getPosition")
        }
...
getDimensions:function(){
    var w,x;
    if(!s.status.rendered){
        return f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.TOOLTIP_NOT_RENDERED,"getDimensions")
        }
...
function p(){
    ...
    s.elements.tooltip='<div qtooltip="'+s.id+'" class="qtooltip '+(s.options.style.classes.tooltip||s.options.style)+'"style="display:none; -moz-border-radius:0; -webkit-border-radius:0; border-radius:0;position:'+s.options.position.type+';">  <div class="qtooltip-wrapper" style="position:relative; overflow:hidden; text-align:left;">    <div class="qtooltip-contentWrapper" style="overflow:hidden;">       <div class="qtooltip-content '+s.options.style.classes.content+'"></div></div></div></div>';
    s.elements.tooltip=f(s.elements.tooltip);
    s.elements.tooltip.appendTo(s.options.position.container);
    s.elements.tooltip.data("qtooltip",{
        current:0,
        interfaces:[s]
        });
    ...
    f.fn.qtooltip.log.error.call(s,2,f.fn.qtooltip.constants.CANVAS_VML_NOT_SUPPORTED,"render")
    }
    ...
    if(typeof s.elements.target.attr("title")=="string"&&s.elements.target.attr("title").length>0){
        ...
        if(typeof s.elements.target.attr("alt")=="string"&&s.elements.target.attr("alt").length>0){
            ...
            u=" ";
            f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.NO_VALID_CONTENT,"render")
            }
        }
}
if(s.options.content.title.text!==false){
...
if(s.options.show.ready===true){
    ...
    s.onRender.call(s);
f.fn.qtooltip.log.error.call(s,1,f.fn.qtooltip.constants.EVENT_RENDERED,"render")
}
function m(){
    var F,z,t,B,x,E,u,G,D,y,w,C,A,s,v;
    F=this;
    F.elements.wrapper.find(".qtooltip-borderBottom, .qtooltip-borderTop").remove();
...
y='<div class="qtooltip-betweenCorners" style="height:'+B+"px; width:"+D+"px; overflow:hidden; background-color:"+x+'; line-height:0.1px; font-size:1px;">';
w='<div class="qtooltip-borderTop" dir="ltr" style="height:'+B+"px; margin-left:"+B+'px; line-height:0.1px; font-size:1px; padding:0;">'+u.topLeft+u.topRight+y;
F.elements.wrapper.prepend(w);
C='<div class="qtooltip-borderBottom" dir="ltr" style="height:'+B+"px; margin-left:"+B+'px; line-height:0.1px; font-size:1px; padding:0;">'+u.bottomLeft+u.bottomRight+y;
...
function l(){
    ...
    if(t.options.hide.fixed){
        ...
        function y(z){
            ...
            t.timers.inactive=setTimeout(function(){
                f(s).each(function(){
                    u.unbind(this+".qtooltip-inactive");
                    t.elements.content.unbind(this+".qtooltip-inactive")
                    });
                t.hide(z)
    ...
    if(t.options.hide.fixed===true){
        t.elements.tooltip.bind("mouseover.qtooltip",function(){
            if(t.status.disabled===true){
                return
            }
...
function x(z){
    ...
    if(t.options.hide.when.event=="inactive"){
        f(s).each(function(){
            u.bind(this+".qtooltip-inactive",y);
            t.elements.content.bind(this+".qtooltip-inactive",y)
            });
        y()
        }
    ...
    if(t.options.hide.fixed===true&&t.options.hide.when.event.search(/mouse(out|leave)/i)!==-1&&f(z.relatedTarget).parents("div.qtooltip[qtooltip]").length>0){
        z.stopPropagation();
        z.preventDefault();
        clearTimeout(t.timers.hide);
        return false
    ...
    t.cache.toggle=0;
    v.bind(t.options.show.when.event+".qtooltip",function(z){
        if(t.cache.toggle==0){
            x(z)
            }else{
            w(z)
    ...
    v.bind(t.options.show.when.event+".qtooltip",x);
    if(t.options.hide.when.event!=="inactive"){
        u.bind(t.options.hide.when.event+".qtooltip",w)
        }
    }
if(t.options.position.type.search(/(fixed|absolute)/)!==-1){
    t.elements.tooltip.bind("mouseover.qtooltip",t.focus)
    }
    if(t.options.position.target==="mouse"&&t.options.position.type!=="static"){
    v.bind("mousemove.qtooltip",function(z){
        t.cache.mouse={
            x:z.pageX,
            y:z.pageY
            };
...
function o(u,v,A){
    ...
    t={
        left:(s.left<f.fn.qtooltip.cache.screen.scroll.left),
        right:(s.left+A.dimensions.width+2>=f.fn.qtooltip.cache.screen.width+f.fn.qtooltip.cache.screen.scroll.left),
        top:(s.top<f.fn.qtooltip.cache.screen.scroll.top),
        bottom:(s.top+A.dimensions.height+2>=f.fn.qtooltip.cache.screen.height+f.fn.qtooltip.cache.screen.scroll.top)
        };
...
function a(){
    ...
    while(typeof x[0].name=="string"){
        x.unshift(c(f.fn.qtooltip.styles[x[0].name]))
        }
        x.unshift(true,{
        classes:{
            tooltip:"qtooltip-"+(arguments[0].name||"defaults")
            }
        },f.fn.qtooltip.styles.defaults);
...
function k(){
    ...
    s=this;
    u=s.getDimensions();
    t='<iframe class="qtooltip-bgiframe" frameborder="0" tabindex="-1" src="javascript:false" style="display:block; position:absolute; z-index:-1; filter:alpha(opacity=\'0\'); border: 1px solid red; height:'+u.height+"px; width:"+u.width+'px" />';
    s.elements.bgiframe=s.elements.wrapper.prepend(t).children(".qtooltip-bgiframe:first")
    }
...
f(window).bind("resize scroll",function(t){
    ...
    s=setTimeout(function(){
        if(t.type==="scroll"){
            f.fn.qtooltip.cache.screen.scroll={
                left:f(window).scrollLeft(),
                top:f(window).scrollTop()
                }
            }else{
        f.fn.qtooltip.cache.screen.width=f(window).width();
        f.fn.qtooltip.cache.screen.height=f(window).height()
        }
        for(i=0;i<f.fn.qtooltip.interfaces.length;i++){
        var u=f.fn.qtooltip.interfaces[i];
...
f(document).bind("mousedown.qtooltip",function(t){
    if(f(t.target).parents("div.qtooltip").length===0){
        f(".qtooltip[unfocus]").each(function(){
            var u=f(this).qtooltip("api");
...
f.fn.qtooltip.constants={};

f.fn.qtooltip.defaults={
    content:{
        prerender:false,
        text:false,
        url:false,
...
f.fn.qtooltip.styles={
    defaults:{
        ...
        classes:{
            target:"",
            tip:"qtooltip-tip",
            title:"qtooltip-title",
            button:"qtooltip-button",
            content:"qtooltip-content",
            active:"qtooltip-active"
        }
    },
cream:{
    border:{
