...
namespace System.Web.Mvc
{
    internal class PagerBuilder
    {
        ...
        internal string RenderPager()
        {
            ...
            return pagerScript + tb.ToString(TagRenderMode.Normal);
        }
            ...
            if (_pagerOptions.UseJqueryAjax)
            {
                ...
                return string.IsNullOrEmpty(url)
                           ? String.Format(CultureInfo.InvariantCulture,
                                           "<li class='active'><a>{0}</a></li>",item.Text)
                           : String.Format(CultureInfo.InvariantCulture,
                                           "<li><a href=\"{0}\" onclick=\"{1}\">{2}</a></li>",
        ...
        private string GeneratePagerElement(PagerItem item)
        {
            ...
            if (item.Disabled) //first,last,next or previous page
                return CreateWrappedPagerElement(item, String.Format("<li class=\"disabled\"><a disabled=\"disabled\">{0}</a></li>", item.Text));
        ...
        private string GenerateMsAjaxPagerElement(PagerItem item)
        {
            if (item.PageIndex == _pageIndex && !item.Disabled) //current page index
                return CreateWrappedPagerElement(item,String.Format("<li>{0}</li>", item.Text));
            if (item.Disabled)
                return CreateWrappedPagerElement(item, string.Format("<li class=\"disabled\"><a disabled=\"disabled\">{0}</a></li>", item.Text));
